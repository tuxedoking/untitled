...
if __name__ == '__main__':
    try:
        ...
        for index in df.index:
            ...
            print(ts_code)
            df_day_line = ds.get_day_line(ts_code, start_date=start_date)
            df_day_line = df_day_line.dropna(subset=['close'])
            print(df_day_line)
            df_pma_s = df_day_line.loc[:, 'close'].to_numpy()
            pmas.cal_pma_s_in_data_frame(df_pma_s, [5, 10, 20, 30, 60, 120, 250])
            db[ts_code] = pickle.dumps(df_day_line)
