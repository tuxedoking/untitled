...
if __name__ == '__main__':
    try:
        db = rocksdb.DB("monthline.db", rocksdb.Options(create_if_missing=True))
        ds = dsts.Datasource()
        df = ds.get_code_list()
        for row in df.itertuples():
            ...
            if data is not None:
                ...
                monthlines = {}
            df2 = ds.get_monthline(row.ts_code, '19800901')
            if df2 is None:
                continue
            ...
            db.put(bytes(row.ts_code, encoding='utf8'), pickle.dumps(monthlines))
